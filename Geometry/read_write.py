    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if values[0] == 'cao_optics:':
                cao_optics = " ".join(values[1:])
            if values[0] == 'cao_kuan:':
...
def GeometryParameters_index(index, gp, temp_para, rank):
    ...
    with open(file_path, 'a') as file:
        ...
        for name, value in variables.items():
            ...
            elif name == 'cao_optics':  file.write(gp[11] + ' ')
            elif name == 'cao_kuan':  file.write(gp[12] + ' ')
            elif name == 'cao_height':  file.write(gp[13] + ' ')
...
def get_all_path(cwd, colling_type):
    ...
    return geometry_path, geometry_script_path
